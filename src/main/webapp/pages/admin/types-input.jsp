...
<script>
  $(function () {
    $("#update").click(function () {
      $("#category-form").attr("action","${pageContext.request.contextPath}/admin/category/update.do");
      $("#category-form").submit();
    })

    $("#save").click(function () {
      $("#category-form").attr("action","${pageContext.request.contextPath}/admin/category/save.do");
      $("#category-form").submit();
    })
  })
...
<nav class="ui inverted gird-header" >
  ...
  <div class="ui attached pointing menu">
    <div class="ui container">
      <div class="right menu">
        <a href="${pageContext.request.contextPath}/admin/category/input.do" class="teal active item">新增</a>
        <a href="${pageContext.request.contextPath}/admin/categories.do?pageNum=1" class="item">列表</a>
      </div>
    </div>
  </div>
  </div>
  ...
  <div  class="m-container-small m-padded-tb-massive">
    <div class="ui container">
      <form id="category-form" action="#" method="post" class="ui form">
        <input type="hidden" name="id" value="${category.id}">
        <div class=" field">
          <div class="ui left labeled input">
            <label class="ui teal basic label">名称</label>
            <input type="text" name="name" placeholder="分类名称" value="${category.name}">
          </div>
        </div>
...
<%--        <div class="ui mini negative message">提示：不能添加重复的分类</div>--%>
        <div class="ui right aligned container">
          ...
          <c:if test="${type==1}"><button class="ui teal submit button" id="update">保存</button></c:if>
          <c:if test="${type==2}"><button class="ui teal submit button" id="save">提交</button></c:if>
        </div>
      </form>
    </div>
  </div>
